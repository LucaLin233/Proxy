...
测试未写入 Profile 的新节点：用 `scripts/test_policy_descriptor.py`（经 `POST /v1/scripting/evaluate` 传 `policy-descriptor`，不改配置）。描述符含密码/PSK，与接口认证是两回事：描述符经 stdin 或 `--descriptor-file -` 送入（不落盘、不写入配置、不读凭据文件），`SURGE_API_KEY` 只用于认证，不作描述符来源。本机 Surge 被 Suspend 时可能返回 `EOF`/超时。
